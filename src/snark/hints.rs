...
pub fn hintgen(gd: &GlobalData, sk: &F, n: usize, i: usize) -> Result<Hint, HintsError> {
    println!("hintgen: n = {}, i = {}", n, i);
    if n == 0 || n & (n - 1) != 0 {
    ...
    for j in 0..n {
        let num = if i == j {
            ...
            let l_j_of_x = &gd.cache.lagrange_polynomials[j];
            l_j_of_x * &l_i_of_x
        };
        let f = &num / &z_of_x;
        let sk_f_poly = utils::poly_eval_mult_c(&f, &sk);
...
pub fn finish_setup(
    ...
    if keys.len() > max_n {
        return Err(HintsError::InvalidInput(
            "More participants than committee size n".to_string(),
        ));
    }
    ...
    if max_n + 1 > gd.params.powers_of_h.len() {
        return Err(HintsError::PolynomialDegreeTooLarge);
    }
    ...
    let hint_aug = hintgen(&gd, &F::zero(), max_n, max_n - 1)?;
    let pk_aug = (gd.params.powers_of_g[0] * F::zero()).into_affine();
    let params = &gd.params;
    ...
    for (i, (pk, hint)) in keys
        ...
        .enumerate()
    {
        if hint.com_q1_contributions.len() != max_n {
        ...
        let l_i_tau_g2 = gd.cache.lagrange_coms_g2[i];

        let lhs = Curve::pairing(pk, l_i_tau_g2);
    ...
    for (i, hint) in hints.iter().enumerate() {
        if !failed_hint_indices.contains(&i) {
            ...
            let com_li_tau_g1 = &gd.cache.lagrange_coms_g1[i];
    ...
    let w_of_x = utils::compute_poly(&weights, &F::zero(), max_n)?;
    let w_of_x_com = KZG::commit_g1(&params, &w_of_x).unwrap();
    let z_of_x = utils::compute_vanishing_poly(max_n);
    let x_monomial = utils::compute_x_monomial();
    let l_n_minus_1_of_x = &gd.cache.lagrange_polynomials[max_n - 1];
