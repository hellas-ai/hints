...
use crate::{snark::*, PublicKey, SecretKey};
...
pub fn hintgen(gd: &GlobalData, sk: &SecretKey, n: usize, i: usize) -> Result<Hint, HintsError> {
    if n == 0 || n & (n - 1) != 0 {
...
pub fn finish_setup(
    ...
    if domain_max + 1 > gd.params.powers_of_h.len() {
        return Err(HintsError::PolynomialDegreeTooLarge);
    }
    ...
    for (i, (pk, hint)) in keys
        ...
        .enumerate()
    {
        if hint.com_q1_contributions.len() != domain_max {
